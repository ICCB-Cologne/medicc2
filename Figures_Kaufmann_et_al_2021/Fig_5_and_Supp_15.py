...
samples_tree = np.array([x.name for x in final_tree.get_terminals()])
samples_df = input_df.index.get_level_values('sample_id').unique()
print(np.sort(samples_tree)[:5])
print(np.sort(samples_df)[:5])
